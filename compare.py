...
def compare_models(rnn_neurons, rnn_activation, rnn_learning_rate, gru_neurons, gru_activation, gru_learning_rate, lstm_neurons, lstm_activation, lstm_learning_rate):
    ...
    for train_index, test_index in kf.split(scaled_prices):
        ...
        # Train the models
        model_rnn.fit(train_data, train_target, epochs=100, batch_size=64, validation_data=(test_data, test_target))
        model_gru.fit(train_data, train_target, epochs=100, batch_size=64, validation_data=(test_data, test_target))
        model_lstm.fit(train_data, train_target, epochs=100, batch_size=64, validation_data=(test_data, test_target))
